...
gemFarmTitle.Text = "💎 AUTO GEM FARM\nAutomatically ends raids at target wave (Continuous)"
...
setWaveBtn.MouseButton1Click:Connect(function()
    ...
    if newWave and newWave > 0 then
        ...
        if autoGemFarmActive then
            autoGemFarmStatus.Text = string.format("Target set to %d - Will end raids at this wave", targetEndWave)
        end
        task.wait(2)
        if autoGemFarmActive then
            autoGemFarmStatus.Text = string.format("Monitoring - Will end raids at wave %d", targetEndWave)
